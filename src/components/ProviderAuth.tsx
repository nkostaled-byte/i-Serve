...
export const ProviderAuth: React.FC<ProviderAuthProps> = ({ onLoginSuccess, onGoToRegister, onBack }) => {
  const [email, setEmail] = useState('john.smith@iserve.app');
  ...
  return (
    <div className="min-h-screen bg-[#F6F8FB] dark:bg-[#070B14] flex flex-col justify-between px-6 pb-6 pt-[calc(env(safe-area-inset-top)+1.5rem)] max-w-md mx-auto relative transition-colors">
      ...
      <div className="flex items-center justify-between pt-4">
        ...
        <span className="text-xs font-semibold text-[#3F73C7] dark:text-[#21C7F6] bg-[#3F73C7]/10 dark:bg-[#21C7F6]/20 px-3 py-1 rounded-full flex items-center space-x-1">
      ...
      >
        ...
        <p className="text-slate-500 dark:text-[#B8C3D9] text-sm mt-1 font-sans mb-6">
          Access instant dispatch jobs, track earnings, and manage your team.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-[#B8C3D9] mb-1.5 ml-1">Provider Email</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-50 dark:bg-[#0E1628] border border-slate-100 dark:border-white/[0.06] rounded-[20px] text-sm text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#3F73C7]/50 transition-all font-sans"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-4 bg-gradient-to-r from-[#3F73C7] to-[#4340A8] dark:from-[#5B6DFF] dark:to-[#3F73C7] text-white font-medium rounded-[20px] float-shadow flex items-center justify-center space-x-2 text-base active:scale-[0.98] transition-transform mt-2"
          >
            {isLoading ? (
        ...
        <div className="mt-6 text-center text-xs text-slate-500 dark:text-[#B8C3D9] font-sans">
          ...
          <button 
            ...
            className="text-[#3F73C7] dark:text-[#21C7F6] font-semibold hover:underline"
          >
      ...
      <div className="text-center text-xs text-slate-400 dark:text-[#7F8DA8] pb-4 font-sans">
        Zero Commission on your first 10 dispatched jobs.
      </div>
    </div>
  );
};
